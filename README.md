...
- **動作**：以圖示方塊呈現可選動作（普攻、重擊、閃避、共鳴技能、共鳴解放），點擊圖示即可選取。
  - **左鍵點擊**：一般選取該動作。
  - **右鍵點擊「共鳴技能」或「共鳴解放」**：標記為「長按」（例如長按共鳴技能），方塊會顯示橘色虛線外框以區分一般選取。
- **備註**：自由輸入文字。若輸入 `*數字`（例如 `*5`），會自動附加在下方「排軸順序預覽」與複製文字的動作名稱後面，方便標記重複次數，例如「普攻 *5」。
- **操作**：
  - `＋`：在該列下方插入新的一列。
  - `⧉`：複製該列內容到下一列。
...
- **攻擊圖示**：可針對每個角色個別上傳專屬的動作圖示（普攻、重擊、閃避、共鳴技能、共鳴解放），上傳後會覆蓋預設的通用動作圖示；未上傳則自動使用系統內建的通用圖示。
